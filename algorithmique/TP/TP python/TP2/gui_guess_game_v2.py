...
class NumberGuessingGameGUI:
    def __init__(self):
        ...
        # Player tracking
        self.players = {}  # {name: {"status": "playing/finished", "attempts": number}}
    ...
    def start_receiver_thread(self):
        """Start a thread to receive updates about other players"""
        self.receiver_thread = threading.Thread(target=self.receive_updates, daemon=True)
        ...
        while self.connected:
            try:
                ...
                try:
                    ...
                    if "player_update" in update:
                        self.players = update["player_update"]
                    if "feedback" in update:
                        self.feedback = update["feedback"]
                    if "attempts" in update:
                        self.attempts = update["attempts"]
                    if "guessed the correct number" in update.get("feedback", ""):
                        self.game_over = True
    ...
    def draw_player_list(self):
        ...
        header = self.small_font.render("Players:", True, BLACK)
        ...
        for name, info in self.players.items():
            ...
            if status == "finished":
                color = GREEN
                text = f"{name}: Finished in {attempts} tries"
            else:
                color = BLUE
                text = f"{name}: Playing..."
            
            player_text = self.small_font.render(text, True, color)
            self.screen.blit(player_text, (20, y_offset))
            y_offset += 25
        
        # Draw current player's attempts
        if not self.game_over:
            attempts_text = self.small_font.render(f"Your attempts: {self.attempts}", True, BLUE)
            self.screen.blit(attempts_text, (20, WINDOW_HEIGHT - 40))
